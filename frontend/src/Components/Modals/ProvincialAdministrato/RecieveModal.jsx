...
const normalizeCode = (raw) => {
  ...
  // Prefer DTRS-… if QR embeds extra text/URL
...
const RecieveModal = ({ open, onClose, onReceived }) => {
  ...
  const startScanner = useCallback(async () => {
    ...
    // Ensure the scanner mount point exists
  ...
  useEffect(() => {
    ...
    // Wait a tick so the scanner region is in the DOM
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-[#607796]/15 bg-[#607796]/5 p-4 space-y-2.5 text-sm">
              ...
              <div className="flex justify-between gap-3">
                ...
                <span className="text-right text-[#3f5168]">{foundDoc.sender}</span>
              </div>
              <div className="flex justify-between gap-3">
