...
const RecieveModal = ({ open, onClose, onReceived }) => {
  ...
  const canReceive =
    ...
    foundDoc?.currentLocation === DOCUMENT_LOCATION.GOVERNOR_OFFICE;
  ...
  const handleReceive = async () => {
    ...
    setError(null);
    try {
      const updated = await receiveAtGovernorOffice(foundDoc.id, name);
      toast.success(`Received by ${updated.receivedByName} — For Signature`);
      onReceived?.(updated);
      onClose?.();
    } catch (err) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="governor-receive-modal-title"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-[#607796]/15 bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-3 mb-5">
          <div>
            ...
            <h2
              id="governor-receive-modal-title"
          ...
          <div className="space-y-4">
            ...
            {!canReceive && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                {foundDoc.currentLocation === DOCUMENT_LOCATION.GOVERNOR_OFFICE
                  ? "This document is already at the Governor Office."
                  : "This document has not been forwarded to the Governor Office yet."}
              </div>
            )}
            ...
            {canReceive && (
              <div>
                <label
                  htmlFor="governor-received-by"
                  className="text-xs font-semibold uppercase tracking-wider text-[#a6a08a]"
                >
                  Received by
                </label>
                <input
                  id="governor-received-by"
                  type="text"
                  value={receiverName}
                  onChange={(e) => {
                    setError(null);
                ...
                <p className="mt-1.5 text-xs text-on-surface-variant">
                  Status becomes For Signature and the date/time is recorded on
                  confirm.
                </p>
              </div>
            )}
